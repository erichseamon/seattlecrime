...
x1 <- data.frame(read.csv("/nethome/erichs/latentplots/class5_old_rev2.csv"))

#--removes th id field, occurant for some csv files masha provides lol

#x1 <- x1[,-1]
colnames(x1)[16] <- "class"
colnames(x1)[14] <- "sth"
...
plot(classt[1,], pch=21, col="gray", xaxt = "n", ylim=c(0,7),
     main="Figure 1b. 5-profile solution for the older subsample", adj=0,
     xlab="",
     ylab="")
     ...
     mtext("IBQ-R Scales: act = activity, sl = smiling/laughter, hp = high intensity pleasure, ps = perceptual sensitivity", side = 1, line = 3)
     mtext("app = approach, nvr = vocal reactivity, dl = distress to limitations, fear = fearfulness, sad = sadness", side=1, line=4) 
     mtext("fall = falling reactivity, do = duration of orienting, lp = low intensity pleasure, sth = soothability, cudd = cuddliness", side=1, line=5)
...
points(classt[4,], pch = 24, col="gray", cex=1.2)
lines(classt[4,], lty=4, lwd=3, col="gray")
points(classt[5,], pch = 25, col="gray", cex=1.2)
lines(classt[5,], lty=6, lwd=3, col="gray")

#--add a legend

legend("bottomright", c("Profile 1: Low Positive/Orienting", "Profile 2: Fearless", "Profile 3: Low Soothable/Cuddly", "Profile 4: High Active/Fearful", "Profile 5: High Positive/Regulated"), lty=c(1,2,3,4,6), lwd=c(2,2, 2,2, 2,2, 2,2, 2,2), seg.len = 10, cex=1, col=c("gray", "gray", "gray", "gray", "gray"), pch=c(21,22,23,24,25), bg='white',ncol=1)
